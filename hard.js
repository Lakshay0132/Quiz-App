  ...
  fetch("./hard.json")
    ...
    .then(function (data) {
      ...
      for(j = 0; j < 4; j++) {
        ...
        document.getElementById("options").innerHTML += `<p class="opt" >${a[j]}</p>`;          //to get the questions from the file
      }
      document.getElementById("question").innerHTML = ` Q${c}. ${data.questions[i]} `;   //to get the options from the file
    });
}
...
function Next() {
  ...
  if (i < 4) {
    ...
    document.getElementById("hard_body").innerHTML =
